...
void print_memory(const multiboot_info_t* mbi){
	...
	if (fmt_int(mbi->mmap_length, tmp, sizeof tmp)) {
	    strlcat(mem, tmp, sizeof mem);
	    strlcat(mem, " MiB total", sizeof mem);
	}

	char tmp2[64] = "";
	if (fmt_int(mbi->mmap_addr, tmp2, sizeof tmp2)) {
	...
	char tmp3[64] = "";
	int bound = mbi->mmap_addr + (mbi->mmap_length << 10);
	if (fmt_int(bound, tmp3, sizeof tmp3)) {
		strlcat(mem, " , ", sizeof mem);
...
/////////////////////preguntar no funciona cambiar tamanio memoria con  -m 256
